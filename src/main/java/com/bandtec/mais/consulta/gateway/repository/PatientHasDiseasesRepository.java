...
public interface PatientHasDiseasesRepository extends JpaRepository<PatientHasDisease, PatientHasDiseaseKey> {

    @Query("SELECT d.paciente.alergias FROM PacienteHasDoencas d WHERE d.paciente.idPaciente = :id")
    Set<PatientHasDisease> findMedicineByPatientId(@Param("id") Integer id);
