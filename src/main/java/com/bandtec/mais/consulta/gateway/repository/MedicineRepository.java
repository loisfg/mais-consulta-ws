...
public interface MedicineRepository extends JpaRepository<Medicine, Integer> {
    ...
    Optional<Set<Medicine>> findByNameStartingWithIgnoreCase(String name);

    @Query("SELECT r.remedio FROM PacienteHasRemedios r WHERE r.paciente.idPaciente = :id")
    List<Medicine> findByPatientId(@Param("id") Integer id);

    @Query("select r from Remedio r where r.id = ?1")
    List<Medicine> findMedicineById(Iterable<Integer> ids);
}
