...
public interface PacienteHasDeficienciaRepository extends JpaRepository<PatientHasDeficiency, PatientHasDeficiencyKey> {
    @Query("SELECT d.paciente.alergias FROM PacienteHasDeficiencia d WHERE d.paciente.idPaciente = :id")
    Set<PatientHasDeficiency> findRemedioByIdPaciente(@Param("id") Integer id);
